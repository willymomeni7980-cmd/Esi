...
def init_db():
    ...
    # پلن‌های داینامیک (ادمین از پنل اضافه/حذف می‌کنه)
...
def set_tutorial(category, file_id, caption=''):
    conn = get_conn(); c = conn.cursor()
    c.execute("DELETE FROM tutorials WHERE category=?", (category,))
    c.execute("INSERT INTO tutorials(category,file_id,caption) VALUES(?,?,?)", (category, file_id, caption or ''))
    conn.commit(); conn.close()

def get_tutorial(category):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM tutorials WHERE category=?", (category,))
    row = c.fetchone(); conn.close()
    return dict(row) if row else None

def delete_tutorial(category):
    conn = get_conn(); c = conn.cursor()
    c.execute("DELETE FROM tutorials WHERE category=?", (category,))
    conn.commit(); conn.close()
...
def get_sales_report(period='daily'):
    ...
    conn = get_conn(); c = conn.cursor()
    if period == 'daily':
        date_filter = "date(confirmed_at) = date('now')"
    elif period == 'weekly':
        date_filter = "confirmed_at >= datetime('now', '-7 days')"
    elif period == 'monthly':
        date_filter = "confirmed_at >= datetime('now', '-30 days')"
    else:
        date_filter = "1=1"
...
def add_plan(plan_key, name, size, duration, price, plan_type, users='—'):
    conn = get_conn(); c = conn.cursor()
    c.execute("""INSERT OR REPLACE INTO plans(plan_key,name,size,duration,price,plan_type,users)
                 VALUES(?,?,?,?,?,?,?)""",
              (plan_key, name, size, duration, price, plan_type, users or '—'))
    conn.commit(); conn.close()

def get_plans_by_type(plan_type):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM plans WHERE plan_type=? AND is_active=1 ORDER BY price ASC", (plan_type,))
    rows = [dict(r) for r in c.fetchall()]; conn.close()
    return rows

def get_all_active_plans():
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM plans WHERE is_active=1 ORDER BY plan_type, price ASC")
    rows = [dict(r) for r in c.fetchall()]; conn.close()
    return rows

def get_plan_by_key(plan_key):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM plans WHERE plan_key=? AND is_active=1", (plan_key,))
    row = c.fetchone(); conn.close()
    return dict(row) if row else None

def delete_plan(plan_key):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE plans SET is_active=0 WHERE plan_key=?", (plan_key,))
    conn.commit(); conn.close()

def get_all_plans_admin():
    """همه پلن‌ها شامل غیرفعال — برای پنل ادمین"""
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT * FROM plans ORDER BY plan_type, price ASC")
    rows = [dict(r) for r in c.fetchall()]; conn.close()
    return rows

def restore_plan(plan_key):
    conn = get_conn(); c = conn.cursor()
    c.execute("UPDATE plans SET is_active=1 WHERE plan_key=?", (plan_key,))
    conn.commit(); conn.close()

def get_referral_earnings(user_id):
    """مجموع کمیسیون‌های دریافتی از زیرمجموعه‌ها (تخمینی از تاریخچه موجودی نداریم، ولی می‌تونیم از payments حساب کنیم)"""
    conn = get_conn(); c = conn.cursor()
    # زیرمجموعه‌های این کاربر
    c.execute("SELECT user_id FROM users WHERE referred_by=?", (user_id,))
    sub_ids = [r["user_id"] for r in c.fetchall()]
    if not sub_ids:
        conn.close()
        return 0
