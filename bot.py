...
async def on_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(uid) and w == "a_upload_tutorial" and (msg.video or has_photo or has_doc):
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif d == "a_del_tutorial_v2ray" or d == "a_del_tutorial_wireguard" or d == "a_del_tutorial_openvpn":
        category = d[14:]
        if not is_admin(uid): return
        db.delete_tutorial(category)
        await q.edit_message_text(f"✅ آموزش حذف شد.", reply_markup=back_kb())
