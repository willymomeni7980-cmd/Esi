...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ref and db_user["_is_new"]:
        ...
        if ref_owner:
            ...
            if flag("referral_open") and updated and updated["referral_count"] >= config.REFERRAL_THRESHOLD and not updated["referral_rewarded"]:
                ...
                if cfg:
                    try:
                        ...
                        logger.error(f"[REFERRAL] send error: {e}")
                else:
...
async def show_category_plans(q, uid, category):
    ...
    if category == "v2ray":
        ...
        plans = config.PLANS  # گیمینگ از همون پلن‌های v2ray استفاده می‌کنه
        icon = "🎮"
...
async def a_show_configs(q, uid):
    ...
    text_plans = list(config.PLANS.keys()) + list(config.TEST_PLANS.keys()) + ["100mb_referral"]
...
async def show_referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u = db.get_user(uid)
    if not u: return
    if not flag("referral_open"):
        await update.message.reply_text("❌ *سیستم رفرال در حال حاضر غیرفعال است.*", parse_mode="Markdown")
        return
