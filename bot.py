...
def price(key):
    ...
    if db_val:
        return int(db_val)
    # پلن داینامیک از دیتابیس
    db_plan = db.get_plan_by_key(key)
    if db_plan:
        return db_plan["price"]
...
def get_plan(key):
    ...
    if db_plan:
        return {
            ...
            "price": db_plan["price"],
            "type": db_plan["plan_type"],
            "users": db_plan.get("users", "—"),
        }
    # fallback به config
...
def is_file_plan(key):
    ...
    db_plan = db.get_plan_by_key(key)
    if db_plan:
        return db_plan["plan_type"] in ("openvpn", "wireguard")
...
def admin_kb():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📋 مدیریت پلن‌ها", callback_data="a_plans")],
    ])
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if w:
        ...
        if w == "a_new_plan_name":      await a_recv_new_plan_name(update, context); return
        if w == "a_new_plan_size":      await a_recv_new_plan_size(update, context); return
        if w == "a_new_plan_duration":  await a_recv_new_plan_duration(update, context); return
        if w == "a_new_plan_price":     await a_recv_new_plan_price(update, context); return
        cs(uid)
...
async def show_category_plans(q, uid, category):
    ...
    cat_map = {
        "v2ray":     ("🌐 *پلن‌های V2ray-Happ*",     "🌐", config.PLANS),
        "gaming":    ("🎮 *پلن‌های Gaming*",           "🎮", config.GAMING_PLANS),
        "wireguard": ("🔐 *پلن‌های WireGuard*",        "🔐", config.WIREGUARD_PLANS),
        "openvpn":   ("🛡 *پلن‌های Open VPN*",         "🛡", config.OPENVPN_PLANS),
    }
    if category not in cat_map:
        return

    title, icon, static_plans = cat_map[category]

    # ترکیب پلن‌های ثابت config + پلن‌های داینامیک دیتابیس
    combined = {}
    for k, p in static_plans.items():
        combined[k] = {"name": p["name"], "price": price(k)}
    for dp in db.get_plans_by_type(category):
        combined[dp["plan_key"]] = {"name": dp["name"], "price": dp["price"]}

    kb = []
    for key, plan in sorted(combined.items(), key=lambda x: x[1]["price"]):
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── آپلود آموزش از ادمین
        ...
        await q.edit_message_text(
            ...
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🌐 V2ray",    callback_data="a_addplan_v2ray")],
                [InlineKeyboardButton("🎮 Gaming",   callback_data="a_addplan_gaming")],
                [InlineKeyboardButton("🔐 WireGuard",callback_data="a_addplan_wireguard")],
                [InlineKeyboardButton("🛡 OpenVPN",  callback_data="a_addplan_openvpn")],
                [InlineKeyboardButton("🔙 بازگشت",   callback_data="a_plans")],
            ])
        )
    elif d.startswith("a_addplan_"):
        plan_type = d[10:]
        if not is_admin(uid): return
        ss(uid, {"w": "a_new_plan_name", "plan_type": plan_type})
        type_labels = {"v2ray": "V2ray", "gaming": "Gaming", "wireguard": "WireGuard", "openvpn": "OpenVPN"}
        await q.edit_message_text(
            f"➕ *افزودن پلن {type_labels.get(plan_type, plan_type)}*\n\n"
            f"*مرحله ۱/۴* — نام پلن را وارد کنید:\n_(مثال: اشتراک ۲۰ گیگ)_",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ لغو", callback_data="a_plans")]]),
        )
    elif d.startswith("a_delplan_"):
        plan_key = d[10:]
        if not is_admin(uid): return
        db.delete_plan(plan_key)
        await q.answer("✅ پلن غیرفعال شد.")
        await a_show_plans(q, uid)
    elif d.startswith("a_restoreplan_"):
        plan_key = d[14:]
        if not is_admin(uid): return
        db.restore_plan(plan_key)
        await q.answer("✅ پلن فعال شد.")
        await a_show_plans(q, uid)
...
async def a_show_plans(q, uid):
    """پنل مدیریت پلن‌های داینامیک"""
    if not is_admin(uid): return
    plans = db.get_all_plans_admin()
    type_icons = {"v2ray": "🌐", "gaming": "🎮", "wireguard": "🔐", "openvpn": "🛡"}

    text = "📋 *مدیریت پلن‌ها — Safe-Server*\n\n"
    if not plans:
        text += "هیچ پلن داینامیکی ثبت نشده.\n_(پلن‌های config.py همچنان فعال هستن)_"
    else:
        for p in plans:
            icon = type_icons.get(p["plan_type"], "📦")
            status = "✅" if p["is_active"] else "❌"
            text += f"{status} {icon} {p['name']} — {fmt(p['price'])}\n"
            text += f"   📊 {p['size']} | ⏱ {p['duration']}\n"
    ...
    for p in plans:
        ...
        if p["is_active"]:
            kb.append([InlineKeyboardButton(
                f"🗑 حذف: {p['name']}", callback_data=f"a_delplan_{p['plan_key']}"
            )])
        else:
            kb.append([InlineKeyboardButton(
                f"♻️ بازگردانی: {p['name']}", callback_data=f"a_restoreplan_{p['plan_key']}"
            )])
    kb.append([InlineKeyboardButton("➕ افزودن پلن جدید", callback_data="a_add_plan")])
    kb.append([InlineKeyboardButton("🔙 بازگشت", callback_data="a_back")])
    await q.edit_message_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
...
async def a_recv_new_plan_name(update, context):
    uid = update.effective_user.id
    state = gs(uid)
    name = update.message.text.strip()
    if not name:
        await update.message.reply_text("⚠️ نام نمی‌تواند خالی باشد."); return
    ss(uid, {**state, "w": "a_new_plan_size", "plan_name": name})
    await update.message.reply_text(
        f"✅ نام: *{escape_md(name)}*\n\n*مرحله ۲/۴* — حجم را وارد کنید:\n_(مثال: ۲۰ گیگابایت یا نامحدود)_",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ لغو", callback_data="a_plans")]]),
    )

async def a_recv_new_plan_size(update, context):
    uid = update.effective_user.id
    state = gs(uid)
    size = update.message.text.strip()
    if not size:
        await update.message.reply_text("⚠️ حجم نمی‌تواند خالی باشد."); return
    ss(uid, {**state, "w": "a_new_plan_duration", "plan_size": size})
    await update.message.reply_text(
        f"✅ حجم: *{escape_md(size)}*\n\n*مرحله ۳/۴* — مدت زمان را وارد کنید:\n_(مثال: ۱ ماهه، نامحدود، ۳۰ روزه)_",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ لغو", callback_data="a_plans")]]),
    )

async def a_recv_new_plan_duration(update, context):
    uid = update.effective_user.id
    state = gs(uid)
    duration = update.message.text.strip()
    if not duration:
        await update.message.reply_text("⚠️ مدت زمان نمی‌تواند خالی باشد."); return
    ss(uid, {**state, "w": "a_new_plan_price", "plan_duration": duration})
    await update.message.reply_text(
        f"✅ مدت: *{escape_md(duration)}*\n\n*مرحله ۴/۴* — قیمت را به تومان وارد کنید:\n_(مثال: 500000)_",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ لغو", callback_data="a_plans")]]),
    )

async def a_recv_new_plan_price(update, context):
    uid = update.effective_user.id
    state = gs(uid)
    try:
        price_val = int(update.message.text.strip().replace(",", "").replace("،", ""))
        if price_val < 0:
            raise ValueError
    except ValueError:
        await update.message.reply_text("⚠️ قیمت باید یک عدد مثبت باشد."); return

    plan_type  = state.get("plan_type", "v2ray")
    plan_name  = state.get("plan_name", "")
    plan_size  = state.get("plan_size", "")
    plan_dur   = state.get("plan_duration", "")

    # ساختن plan_key یکتا
    import re, time
    base = re.sub(r"[^a-z0-9]", "", plan_name.lower())[:12] or "plan"
    plan_key = f"dyn_{plan_type}_{base}_{int(time.time()) % 100000}"

    db.add_plan(plan_key, plan_name, plan_size, plan_dur, price_val, plan_type)
    cs(uid)

    type_icons = {"v2ray": "🌐", "gaming": "🎮", "wireguard": "🔐", "openvpn": "🛡"}
    icon = type_icons.get(plan_type, "📦")

    await update.message.reply_text(
        f"✅ *پلن جدید اضافه شد!*\n\n"
        f"{icon} نام: {escape_md(plan_name)}\n"
        f"📊 حجم: {escape_md(plan_size)}\n"
        f"⏱ مدت: {escape_md(plan_dur)}\n"
        f"💵 قیمت: {fmt(price_val)}\n\n"
        f"پلن فوری در منوی خرید کاربران نمایش داده می‌شود.",
        parse_mode="Markdown",
        reply_markup=main_kb(uid),
    )
